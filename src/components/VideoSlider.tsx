...
const VideoSlider = ({ videos }: { videos: Video[] }) => {
  ...
  const getUniqueKey = (video: Video, index: number) => video.key || `${video.id}-${index}`;
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {videos.map((video, index) => {
            const uniqueKey = getUniqueKey(video, index);
            return (
              <div key={uniqueKey} className="relative rounded-xl overflow-hidden shadow-card group cursor-pointer aspect-video bg-muted">
                {activeVideo === uniqueKey ? (
                  <iframe
                  ...
                  <div onClick={() => setActiveVideo(uniqueKey)} className="w-full h-full relative">
                    <img
